...
H = [];
angl = [];
for i = 1:numel(source_indx)   
    H = [H;H_T((source_indx(i)-1)*num_d+1:source_indx(i)*num_d,:)];
    angl = [angl;angl_T((source_indx(i)-1)*num_d+1:source_indx(i)*num_d)];
end
H_T = H;
angl_T = angl;
clear H
...
load(['400vanSYM100_' num_ph '_exp5.mat']); %400vanSYM_1e7_exp1
Imin = I;
...
I4 = repmat(Imin(3*num_s+1:4*num_s),num_d,1);
I5 = repmat(Imin(4*num_s+1:5*num_s),num_d,1);
...
r4_m = reshape((I4(:)).*angl_T.*exp(-H_T*f(:)),num_d,num_s);
r5_m = reshape((I5(:)).*angl_T.*exp(-H_T*f(:)),num_d,num_s);        

rm = sum([r1_m;r2_m;r3_m;r4_m;r5_m],2);
...
for i = 1:NumItr
    ...
    q4 = (I4(:)).*angl_T.*exp(-H_T*x_i);
    q5 = (I5(:)).*angl_T.*exp(-H_T*x_i);    
    Q1 = reshape(q1,num_d,num_s);
    Q2 = reshape(q2,num_d,num_s);   
    Q3 = reshape(q3,num_d,num_s);   
    Q4 = reshape(q4,num_d,num_s);       
    Q5 = reshape(q5,num_d,num_s);       
    Q = [Q1;Q2;Q3;Q4;Q5];       
...
%         cd('./fig_journal')
% 	%cd('./fig_train')
%         txt = ['50MUX5_train2_' num_ph '_' lambda_txt '_' num2str(id) '.mat'];
